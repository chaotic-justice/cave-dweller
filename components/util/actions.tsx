...
import { Maybe, PageBlocksHeroActions } from "../../tina/__generated__/types";
...
export const Actions = ({
  ...
  className: string;
  actions: Maybe<PageBlocksHeroActions>[];
}) => {
            ...
            element = (
              <Link key={index} href={action.link ? action.link : "/"}>
                <button
                  ...
                  className={`z-10 relative flex items-center px-7 py-3 font-semibold text-lg transition duration-150 ease-out  rounded-lg transform focus:shadow-outline focus:outline-none focus:ring-2 ring-offset-current ring-offset-2 whitespace-nowrap ${invertedButtonColorClasses.blue}`}
                >
                  {action.label}
                  {action.icon && (
                    <BiRightArrowAlt
